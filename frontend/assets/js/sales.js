...
window.sendBillSmsBackground = async function(billId, btnEl) {
  ...
  try {
    ...
    if (res && res.sent) {
      showToast(res.message || 'SMS dispatched in background!', 'success');
      if (btnEl) {
    ...
    showToast(err.message || 'Failed to dispatch SMS in background', 'error');
    if (btnEl) {
      btnEl.disabled = false;
      btnEl.innerHTML = origHtml;
    }
...
function openBillPrint(bill) {
  ...
  const notifyButtons = cleanPhone ? `
    <div style="background:rgba(59, 130, 246, 0.08);border:1px solid rgba(59, 130, 246, 0.3);padding:12px 14px;border-radius:10px;margin-top:12px;text-align:center">
      <div style="font-weight:600;font-size:13px;color:var(--text-primary);margin-bottom:4px">
        📱 SMS Receipt for Patient (${phone})
      </div>
      <p style="font-size:11.5px;color:var(--text-muted);margin:0 0 10px">
        Automated SMS dispatched in background. You can also trigger or re-send directly:
      </p>
      <div style="display:flex;gap:10px;justify-content:center;flex-wrap:wrap">
        <button type="button" class="btn btn-primary btn-sm" onclick="sendBillSmsBackground(${bill.id}, this)" style="display:inline-flex;align-items:center;gap:6px;padding:7px 18px;border-radius:8px;font-size:12.5px;font-weight:600;cursor:pointer">
  ...
  ` : `
    <div style="background:var(--bg-surface);padding:8px 12px;border-radius:8px;margin-top:10px;border:1px dashed var(--border);text-align:center">
      <p style="font-size:11px;color:var(--text-muted);margin:0">💡 To send automated SMS receipts, enter patient phone before generating bill.</p>
    </div>
  `;
  ...
  if (cleanPhone) {
    modalActions.push({
      label: '📱 Send SMS',
      cls: 'btn-secondary',
      action: (btn) => { sendBillSmsBackground(bill.id, btn); },
    });
  }
  modalActions.push({
